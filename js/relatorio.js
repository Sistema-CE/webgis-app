...
function buildAnmReportSection(){
  ...
  if(!anmResults.length) return '';

  const rows=anmResults.flatMap(result=>result.anmProcesses).map(item=>`
    <tr>
      <td>${escapeHtml(item.processo)}</td>
      <td>${escapeHtml(item.nome)}</td>
      <td>${escapeHtml(item.fase)}</td>
      <td>${escapeHtml(item.substancia)}</td>
      <td>${fmt(item.areaProcesso,2)} ha</td>
      <td>${fmt(item.areaSobreposta,4)} ha</td>
      <td>${fmt(item.percentualAoi,2)}%</td>
    </tr>`).join('');

  const total=anmResults.reduce((sum,result)=>sum+result.anmProcesses.length,0);
  return `
    <h2>Processos Minerários — ANM</h2>
    <p>Foram identificados <b>${total}</b> processo(s) minerário(s) incidente(s) na Área de Interesse.</p>
    <table>
      <thead>
        <tr>
          <th>Processo</th>
